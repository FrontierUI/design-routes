...
const Services = () => {
  return (
    <div className="w-full h-full grid grid-cols-1 xl:grid-cols-3 lg:gap-x-4 max-sm:mr-4 max-sm:space-y-2">
      <div className="flex flex-col gap-y-1 w-full">
        {prod1.map((prods1) => (
          <Link
            key={prods1.id}
            to={prods1.href}
            className="overflow-hidden rounded-lg h-full w-full m-2 transition duration-300 shadow-drop-4 py-1.5"
          >
      ...
      {/* {socialProduct.map((soci) => (
          <Link
            key={soci.id}
            to={soci.href}
            className="overflow-hidden rounded-lg w-full m-2 transition duration-300 shadow-drop-4 py-1.5"
          >
            <div className="p-6 w-full rounded-lg overflow-hidden transitAll scal105 hover:bg-primary bg-white hover:text-white text-black">
              <img src={soci.image} className="img-fluid rounded-lg" alt="" />

              <div className="pt-3 space-y-1">
                <h2 className="monaBold text-xl">{soci.title}</h2>
                <p className="text-sm">{soci.descr}</p>
              </div>
            </div>
          </Link>
        ))}
        {presentationProduct.map((present) => (
          <Link
            key={present.id}
            to={present.href}
            className="overflow-hidden rounded-lg w-full m-2 transition duration-300 shadow-drop-4 py-1.5"
          >
            <div className="p-6 w-full rounded-lg overflow-hidden transitAll scal105 hover:bg-primary bg-white hover:text-white text-black">
              ...
              <div className="pt-3 space-y-1">
                <h2 className="monaBold text-xl">{present.title}</h2>
                <p className="text-sm">{present.descr}</p>
              </div>
            </div>
          </Link>
        ))} */}
